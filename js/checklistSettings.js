/**
 * js/checklistSettings.js (v0.0.40)
 * 설정 탭 — "체크리스트 관리" 섹션
 *
 * 세 가지를 담당함:
...
function rowHtml({ key, icon, label, syncable, deletable }) {
  ...
  return `
    <div class="cl-settings-row">
      ...
      <span class="cl-settings-row-label">${label}</span>
...
export function renderChecklistSettings() {
  ...
  const packRows   = clPacks.map(p => ({ key: p.key, icon: p.icon, label: p.label }));
  const customRows = (S.customChecklists || []).map(c => ({ key: c.key, icon: c.icon || 'checklist', label: c.label, deletable: true }));

  wrap.innerHTML = `
    <div class="cl-settings-hint">아이 단계(임신중/출생 후)에 따라 실제로 보이는 탭은 달라져요 — 여기서는 전체를 한 번에 관리해요.</div>
    <div class="cl-settings-group-label">기본 체크리스트</div>
    ${BUILTIN_ROWS.map(rowHtml).join('')}
    <div class="cl-settings-group-label">준비물 체크리스트</div>
    ${packRows.map(rowHtml).join('')}
    ${customRows.length ? `<div class="cl-settings-group-label">내가 만든 체크리스트</div>${customRows.map(rowHtml).join('')}` : ''}
...
export function openCreateChecklistModal() {
  showModal('새 체크리스트 만들기', `
    ...
    <div class="fg" style="margin-top:10px">
      <label>항목 (한 줄에 하나씩, 나중에 체크리스트 화면에서도 추가할 수 있어요)</label>
...
export function submitCreateChecklist() {
  ...
  S.customChecklists.push({ id, key: `custom_${id}`, label, icon: 'checklist', items });

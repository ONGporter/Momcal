...
- **버전 문자열을 한 곳만 고치고 넘어가지 마** — `index.html`의 `.site-footer-version`과 `scripts/build-guide.mjs`의 `APP_VERSION`은 항상 세트로 수정. (v0.0.15~16에서 이걸 빼먹어 화면 버전이 두 버전 동안 안 올라간 적 있음 — `docs/TODO.md` 참고)
...
## Doc-gardening 체크리스트 (문서 부패 방지 — 자동화 없이 세션 시작 시 수동으로)

이 저장소엔 CI나 자동 문서 검증 봇이 없습니다(빌드 스텝이 없는 정적 사이트라 마련하지 않음). 그래서 "문서가 실제 코드와 어긋나는 것"을 잡아내는 유일한 방법은 **매 세션 시작 시 아래를 훑는 것**뿐입니다. 실제로 이걸 안 해서 버전 표시가 2개 버전 동안 안 맞은 적(v0.0.15~16)과 CHANGELOG 헤더가 깨진 채로 방치된 적(v0.0.19)이 있습니다 — 둘 다 이 체크를 했으면 바로 잡혔을 문제입니다.

**새 세션을 시작할 때, 본 작업 전에 아래를 빠르게 확인하세요** (1~2분이면 됨):
- [ ] `index.html`의 `.site-footer-version`, `scripts/build-guide.mjs`의 `APP_VERSION`, `docs/CHANGELOG.md` 맨 위 항목의 버전이 셋 다 같은가

...
- **`sw.js`의 HTML/JS/CSS는 network-first 전략(v0.3.13부터)** — 예전엔 stale-while-revalidate(캐시 우선 응답)라서 배포 직후 "일반 창에서 새로고침해도 예전 버전이 먼저 뜨고, 한 번 더 새로고침해야 최신본이 보이는" 구조적 문제가 있었음(시크릿 창은 캐시가 없어서 항상 정상으로 보여 원인 파악이 오래 걸림 — v0.3.9~v0.3.13에 걸쳐 여러 번 "배포했는데 안 바뀐다"는 제보로 나타남). 캐싱 전략 자체를 바꿀 땐 이 트레이드오프(오프라인 지원 vs 배포 즉시 반영)를 다시 흔들지 말 것 — 아이콘·폰트는 여전히 캐시 우선 유지

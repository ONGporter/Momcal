...
// v0.0.52: (1) kiss.png 몸통에 구멍 뚫리던 버그 수정 — 얇은 이음새를 타고 배경 제거가 몸통
// 안쪽까지 새어 들어간 게 원인이었음, 라벨링 전 마스크를 침식(erosion)시켜 얇은 다리를 끊고
// 다시 팽창시켜 복원하는 방식으로 전체 10종 재처리. (2) 캘린더 스티커 "꽃·자연" 카테고리를
// 기존 이모지 16종 → 자체 제작 일러스트 25종으로 전면 교체(icons/stickers/nature/*.png 신규).
// ICON_STICKERS를 카테고리별 폴더를 지원하도록 구조 정리(file+공용베이스 → path 필드로 통일)
// — 캐시 버전 상향
// v0.0.53: v0.0.51~52에서 "이모지보다 작아 보인다"는 피드백을 캔버스의 94%까지 꽉 채우는
// 방식으로 고쳤었는데, 그 결과 투명 여백이 너무 얇아져서 실제 앱 표시 크기(약 23px)로 축소하면
// 여백이 안티에일리어싱에 묻혀 사실상 안 보이게 됨 — 캐릭터 자체가 흰색/크림색이라 "배경이
// 다시 흰색으로 복원된 것처럼" 보였던 원인. 픽셀 알파값은 항상 정확했지만(모서리 투명 확인됨),
// 실사용 크기 기준 검증이 빠졌던 게 문제. 채움 비율을 94%→78%로 완화해서 축소 후에도
// 투명 여백이 뚜렷이 보이도록(다운스케일 후 투명 비율 33~53%로 확인) momcal-action 10종 +
// nature 25종 전체 재처리 — 캐시 버전 상향
// v0.0.54: v0.0.51~53을 거치며 매번 "이번엔 진짜 됐다"고 확인했는데도 실제 배포 화면에서는
// 계속 흰 배경으로 보이는 게 반복됨 — 파일을 몇 번을 다시 만들어도 고쳐지지 않는 게 이상해서
// 원인을 재점검한 결과, 애초에 이미지 파일 문제가 아니라 **캐싱 문제**였음이 드러남.
// icons/stickers/momcal-action·nature 아래 파일들이 v0.0.50부터 지금까지 파일명이 단 한 번도
// 안 바뀌었고, 아래 fetch 핸들러가 "캐시에 있으면 그걸 먼저 반환하고 새 버전은 백그라운드로만
// 받아두는" cache-first 방식이라 — CACHE_NAME을 아무리 올려도 브라우저 HTTP 캐시·Vercel CDN
// 엣지 캐시는 URL이 그대로면 계속 예전 바이트를 재사용할 수 있었음(세션마다 결과가 달랐던 이유).
// js/calendar.js의 stickerDisplay()가 이미지 URL 끝에 ?v=ICON_ASSET_VERSION 쿼리스트링을 붙이도록
// 변경 — 내용이 바뀔 때마다 이 값을 올리면 URL 자체가 달라져서 모든 캐시 계층이 무조건 새로
// 받아올 수밖에 없음(앞으로 스티커 이미지 교체 시 이 값도 반드시 함께 올릴 것) — 캐시 버전 상향

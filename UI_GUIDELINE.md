...
- **한계**: "컬러 시스템" 원칙상 색상은 변수로 써야 하지만, 실제 코드에는 `#fff`/`#EEE5F4`/`#FBF6FA`/`#FAFAFA` 같은 하드코딩된 색이 곳곳에 남아있어서 이런 요소들은 `html[data-theme="dark"] .클래스 { ... }` 형태로 하나씩 별도로 덮어써야 했음. v0.0.5에서 topbar·nav pill·로그인 화면 등 1차로 덮었고, v0.0.8에서 실사용 캡처를 받아 `.ad-fallback`(홈 육아 팁), `.ci`(체크리스트 미완료 행), `.gov-cl-item`(정부지원 체크리스트), `.growth-record-item`(성장 기록 목록), `.auth-tabs`, `.progress-bar`, `.sp-tab`(스티커 탭) 등을 추가로 덮음. **새 컴포넌트를 만들 때 하드코딩된 색 대신 CSS 변수를 쓰면, 다크 모드가 별도 작업 없이 자동으로 적용됨** — 이 원칙을 지키는 게 왜 중요한지 보여주는 사례

...
설정 탭(`js/theme.js`)에서 켜고 끌 수 있음. `<html data-theme="dark">` 속성으로 전환되고, `css/main.css` 안의 `html[data-theme="dark"] { ... }` 블록이 색상을 재정의함. 기기별 `localStorage`에 저장되며 Firestore에는 저장되지 않음(로그인해도 다른 기기와 동기화되지 않음).

- **적용 범위**: 앱 본체(`index.html`)만 지원. 육아정보 페이지(`guide/`)는 정적 SEO 페이지라 이번 버전에서는 대상 밖 — 나중에 지원하려면 `guide/guide.css`에도 같은 방식으로 다크 변수를 추가해야 함
- **작동 원리**: 위 "컬러 시스템"의 CSS 변수(`--bg`/`--card`/`--tx`/`--txl`/`--wh` 등)를 다크용 값으로 재정의 — 이 변수들을 쓰는 요소는 대부분 자동으로 전환됨
- **한계**: "컬러 시스템" 원칙상 색상은 변수로 써야 하지만, 실제 코드에는 `#fff`/`#EEE5F4`/`#FBF6FA` 같은 하드코딩된 색이 곳곳에 남아있어서(topbar, nav pill, 로그인 화면, 체크리스트 세부 설명 등) 이런 요소들은 `html[data-theme="dark"] .클래스 { ... }` 형태로 하나씩 별도로 덮어써야 했음. **새 컴포넌트를 만들 때 하드코딩된 색 대신 CSS 변수를 쓰면, 다크 모드가 별도 작업 없이 자동으로 적용됨** — 이 원칙을 지키는 게 왜 중요한지 보여주는 사례
- 새로운 화면/컴포넌트를 추가했는데 다크 모드에서 이상하게 보이면(밝은 배경에 밝은 글씨 등), `html[data-theme="dark"]` 블록에 해당 클래스를 추가해서 보정할 것

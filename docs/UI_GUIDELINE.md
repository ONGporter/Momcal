...
하단 nav-pills는 v0.0.5부터 5개 탭(🏠 홈 / 📅 캘린더 / ✅ 체크리스트 / 📈 성장 / ⚙️ 설정)이며, `.nav-pills`는 `repeat(5, minmax(0,1fr))` 그리드로 균등 분할됨. 탭이 늘어나 모바일에서 더 빠듯해져서 520px·380px 두 단계 반응형으로 글자 크기를 축소함 — 탭을 더 추가할 계획이라면 이 폭 계산을 다시 검토할 것. "설정" 탭에는 다크 모드, 배우자와 함께 쓰기, 알림 설정이 들어있고, "어플로 추가"는 사용자 요청으로 설정이 아니라 홈 화면에 그대로 남아있음(v0.0.5).

...
| 위치 | 색 | 이유 |
|---|---|---|
| 캘린더 이벤트 카테고리(선택/이유식/접종/정부지원/내 일정) | 민트·빨강·보라·초록·파랑 | 일정 종류 구분 (필수는 메인 핑크를 그대로 사용, 예외 아님) |
...
| `rec` | 선택 | `#E0F2F1` | `#26A69A` |
| `food` | 이유식 | *(CSS 변수 없음 — 하드코딩)* | `#E53935` |
| `vax` | 접종 | `#EDE7F6` | `#9575CD` |
| `gov` | 정부지원 | *(CSS 변수 없음 — 하드코딩)* | `#43A047` |
| `custom` | 내일정 | `#E3F2FD` | `#64B5F6` |

> `req`/`rec`/`vax`/`custom`은 `--pkl`/`--mnl`/`--pul`/`--bll` CSS 변수를 씀. `food`·`gov`는 `js/calendar.js`의 `DEFAULT_EV_COLORS`에만 하드코딩되어 있고 대응하는 CSS 변수가 없음 — 새 변수 추가가 필요하면 "컬러 시스템" 섹션에 `--fd`/`--gv` 계열을 먼저 만들 것.
